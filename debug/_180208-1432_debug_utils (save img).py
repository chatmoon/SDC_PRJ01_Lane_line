...
default_header= ['slopes', 'rights', 'lefts', 'y_mins', 'y_min', 'y_max', 'rights_slope', 'rights_intercept', 'rights_x_min', 'rights_x_max', 'lefts_slope', 'lefts_intercept', 'lefts_x_min', 'lefts_x_max']
...
def parse_args():
    ...
    parser.add_argument('-c', '--csv',		 dest='csv_file',	  help='name of the csv file', action='store', type=str, default=default_csv)    
    parser.add_argument('-n', '--header',	 dest='header',		  help='header name stored in the first line of the csv file', action='store', type=list, default=default_header)
    parser.add_argument('-v', '--video',	 dest='video',		  help='video directory path', action='store', type=str, default=default_video)
    parser.add_argument('-t', '--threshold', dest='threshold',	  help='tuple = (low_threshold, threshold, high_threshold)', type=tuple, default=(60, 45, 180)) # canny()
    parser.add_argument('-k', '--kernel',	 dest='kernel_size',  help='kernel size', type=int, default=3) # gaussian_blur()
    parser.add_argument('-r', '--rho',		 dest='rho',		  help='rho', type=int, default=2) # hough_lines()
    parser.add_argument('-a', '--theta',	 dest='theta',		  help='theta', type=float, default=np.pi/180) # hough_lines()
    parser.add_argument('-l', '--len',		 dest='min_line_len', help='min_line_len', type=int, default=50) # hough_lines()
    parser.add_argument('-g', '--gap',		 dest='max_line_gap', help='max_line_gap', type=int, default=150) # hough_lines()
...
def hough_lines(args, img):
    ...
    draw_lines(args, line_img, lines)
    return line_img
