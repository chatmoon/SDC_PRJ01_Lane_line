...
def draw_lines(args, img, lines, color=[255, 0, 0], thickness=8):
    ...
    for lanes in [rights,lefts]:
        ...
        if slope_std == 0:
            ...
            slope = np.mean( [ lane[1] for lane in lanes if lane[1] - slope_mean < 1*slope_std ] )            
        print()
        ...
        if intercept_std == 0:
            ...
            intercept = np.mean( [ lane[2] for lane in lanes if lane[2] - intercept_mean < 1*intercept_std ] )
